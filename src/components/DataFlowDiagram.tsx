...
/**
 * @description Composant principal qui affiche le diagramme de flux de données
 * Ce diagramme illustre le pipeline complet de traitement des données:
 * Source brute → Bronze → Silver → Gold → Modèle sémantique → Power BI
 */
...
const DataFlowDiagram = () => {
  const { setActiveNode, activeNode, isDemo, setIsDemo, addLog } = useDataFlowStore()
  ...
  const onNodeClick = useCallback(
    (event: React.MouseEvent, node: Node) => {
      ...
      addLog(`Nœud sélectionné: ${node.data.label}`)
    },
    [setActiveNode, addLog],
  )
  ...
  useEffect(() => {
    if (isDemo) {
      ...
      addLog("Démarrage de la démonstration...", "info")
      ...
      const interval = setInterval(() => {
        if (currentStage < stages.length) {
          ...
          // Ajouter des logs spécifiques à chaque étape
          switch (stage) {
            case "source":
              addLog("Chargement des données brutes...", "info")
              break
            case "bronze":
              addLog("Nettoyage des données (encodage, séparateurs)...", "success")
              break
            case "silver":
              addLog("Application du typage et normalisation des colonnes...", "success")
              break
            case "gold":
              addLog("Création des agrégations et métriques...", "success")
              break
            case "semantic":
              addLog("Construction du modèle sémantique...", "success")
              break
            case "powerbi":
              addLog("Chargement du rapport Power BI...", "success")
              addLog("Démonstration terminée!", "info")
              setIsDemo(false)
              break
          }
  ...
  }, [isDemo, setActiveNode, addLog, setIsDemo])
